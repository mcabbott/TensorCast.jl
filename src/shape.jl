...
export @shape, @shape!, @reduce, @reduce!
...
function tensor_slice_main(nameZ, indZ, indZsub,
    ...
    V && @info "todo & packages" mustcopy mustnotcopy inplace usingstatic usingstrided icheck
    ...
    if icheck # run such checks only on outermost expr, and they ignore i/j and -i etc.  
        runichecks = [ check_one( :( $nameA[$(indA...)]) ,where) |> esc, 
                        check_one( :( $nameZ[$(indZ...)]) ,where) |> esc]
        if check_options.size
            append!(outex.args, runichecks)
            returnoutex = true
            mustcheck = true
        end
    end
    ...
    if mustcheck && length(storeAZ.checks) > 0
        append!(outex.args, storeAZ.checks)
        returnoutex = true
    end

    if needsizes
        push!(outex.args, :( local sz = $sizeVlistex ))
        returnoutex = true
    end
